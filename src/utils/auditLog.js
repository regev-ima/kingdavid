...
// Writes to the generic `audit_logs` table (entity_name/entity_id).
// Used for Customer field changes (e.g. rep reassignment) since customers
// don't have a dedicated activity log table like leads do.
export async function createCustomerAuditLog({ customerId, actionType, description, user, fieldName, oldValue, newValue, metadata }) {
  try {
