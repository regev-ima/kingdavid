-- ============================================================================
-- Task import: the lead that existed when the task was created; text in the id
-- ============================================================================
-- Two things the full 54,777-row run showed, both on real cards:
--
--   1. One phone, two leads here — an enquiry from February (imported from
--      Kaveret) and a fresh web form in August — where Kaveret had one card.
--      "Newest lead" hung February's two calls on the August lead and left
--      the February card empty. The lead is now the one that existed when
--      the task was created: the newest lead created at or before the task's
--      creation stamp, or the earliest when none predates it. February's
--      calls go to February's lead; the August call to August's.
--
--   2. Kaveret does create two tasks on one lead in the same minute (a note
--      and a call, rows 4893/4894 of the run), so lead + minute folded the
--      second into the first: 367 rows came back "updated" and a few of them
--      were a second task overwriting the first. The text is now part of the
--      identity ('kaveret:v3:…'); the due date stays out, so a rescheduled
--      follow-up still updates rather than duplicates.
--
-- Rows imported under the earlier derivations are still found by them, but
-- the clean path for a batch already imported is: בטל ייבוא, then import the
-- file again — the lead choice for a task cannot be corrected by an UPDATE
-- that matches on the old lead.
--
-- Otherwise byte-identical to 20260902000005.
-- ============================================================================
