-- ============================================================================
-- process_lead_import stops depending on a trigger the bulk runner suppresses
-- ============================================================================
-- The 125,280-row Kaveret import ran at ~35 rows/sec: 2,100 rows in a full
-- minute. The cause is documented in backfill-lead-contacts.yml, which hit it
-- first — public.leads carries auto_update_lead_counters(), and on EVERY row
-- write it runs
--     UPDATE lead_counters SET total_leads = (SELECT COUNT(*) FROM leads …)
-- a full scan of 116k rows. One import row = one full table scan.
--
-- The cure there was SET LOCAL session_replication_role = 'replica', which
-- suppresses user triggers for that transaction only. The new
-- import-leads-run.yml does the same. But suppression is only safe if the
-- function no longer needs the triggers it is switching off:
--
--   trg_leads_attach_contact              already redundant — the import
--                                         resolves the contact itself and puts
--                                         contact_id in the payload.
--   auto_update_lead_counters             self-correcting: it recomputes
--                                         absolutely (SET = COUNT(*)), and the
--                                         workflow nudges one ordinary write at
--                                         the end to force it.
--   leads_activity_log_insert/update      deliberately skipped. 125k activity
--                                         rows describing a bulk backfill is
--                                         noise, not history.
--   trg_leads_effective_sort_date_default THE GAP THIS FILE CLOSES. It fills
--                                         effective_sort_date on INSERT when
--                                         nobody else did. The import only set
--                                         that field when the Kaveret row
--                                         carried a date, so with the trigger
--                                         off, a dateless row would land with
--                                         NULL — and a NULL effective_sort_date
--                                         is excluded by every {$gte,$lte}
--                                         range filter, which is precisely the
--                                         under-counting 20260707000001 fixed.
--
-- So the field is now set unconditionally, with the same COALESCE fallback the
-- trigger used. Nothing else changes; the rest of the body is byte-identical to
-- 20260803000003.
-- ============================================================================
