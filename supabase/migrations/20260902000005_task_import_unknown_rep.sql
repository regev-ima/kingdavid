-- ============================================================================
-- Task import: tasks of reps who left land in a queue someone reads
-- ============================================================================
-- The Kaveret task export names 33 reps; 27 of them no longer have a user
-- here. Imported as-is, a task of theirs keeps an address nobody logs in
-- with — fine for a task already done, which is history and should say who
-- did it, but an OPEN task with that address sits in no one's queue and is
-- simply never seen.
--
-- The browser now flags such rows (rep_unknown) and names the fallback the
-- operator picked (rep_fallback). For an open task — or for every task when
-- the operator asked for that (rep_move_completed) — the rep becomes the
-- lead's current owner here, who inherited the lead from the same person, or
-- else the fallback. Done tasks keep their original address by default.
--
-- Otherwise byte-identical to 20260902000004.
-- ============================================================================
