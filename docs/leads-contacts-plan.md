...
| שלב | מה | הרסני? | סטטוס |
|---|---|---|---|
| **0** | Dump סכימה, אימות גיבוי, קבלת קובץ דוגמה | לא | ⏳ **ממתין לך** (§1) |
| **0.5** | ניקוי מקדים: מחיקת קוד מת, תיקון `'won'`/`'qualified'`, הברחת חיפוש | לא | טרם |
| **1** | תשתית טלפון: `normalize_il_phone`, `phone_normalized` GENERATED, אינדקסים | לא | ✅ **בוצע** |
| **3** | `resolve_or_create_contact` + טריגר על `leads` + `contact_id` | לא | ✅ **בוצע** |
| **6א** | מסך ייבוא לידים בהגדרות: CSV/Excel → staging → מיזוג ב-SQL, עם מזהה כוורת כמפתח עדכון | לא | ✅ **בוצע** |
| **1ב** | מיזוג הכפילויות הקיימות ב-`customers` + הוספת UNIQUE | לא | ⏳ תלוי בשלב 0 |
| **2** | `lead_status_config` + `source_mapping` + טריגר גזירת מקור | לא | טרם |
| **4** | `recompute_contact_status` + טריגרים + backfill | לא | טרם |
| **5** | UI: כרטיס לקוח עם כל הלידים · סטטוס לקוח + פילטר · איתור ליד מקובץ לפי אדם | לא | טרם |
| **6ב** | **האיפוס** — runbook ידני, לא מיגרציה | **כן** | ⏳ אישור שלך |
| **7** | אחרי: בנייה מחדש של מונים, שאילתות אימות, הפעלת אוטומציות | לא | טרם |

### 6.0 מה שכבר נבנה — `20260726000001_lead_import.sql` + מסך הייבוא

**נרמול טלפון בצד ה-DB.** `public.normalize_il_phone()` מחזירה `972XXXXXXXXX`, ו-
`customers.phone_normalized` / `leads.phone_normalized` הן עמודות **`GENERATED ALWAYS ... STORED`**
ולא ממולאות בטריגר. ההבדל מהותי: אי אפשר לכתוב לעמודה מחושבת ואי אפשר לעקוף אותה. כלומר
**כל שבעת נתיבי הכניסה מכוסים בלי לגעת באף אחד מהם** — כולל ה-RPC הציבורי מהאתר וה-webhook
החיצוני. נוספה `is_valid_il_phone()` ו-`customers.phone_valid` לסימון מספרים זרים/שבורים.

**איש קשר לכל ליד.** `resolve_or_create_contact()` מאתרת לפי `phone_normalized` או יוצרת,
ו-`trg_leads_attach_contact` (BEFORE INSERT OR UPDATE OF phone) מפעילה אותה לכל ליד שנכנס.
שם שונה על אותו מספר נשמר ב-`alt_names[]` במקום ליצור איש קשר שני. הפונקציה נועלת
`pg_advisory_xact_lock` על המספר, ולכן נכונה גם בכתיבות מקבילות וגם **לפני** שאילוץ ה-UNIQUE קיים.

**מזהה כוורת כמפתח עדכון.** `leads.external_source` + `leads.external_id` עם אינדקס UNIQUE חלקי.
שורה שחוזרת עם אותו מזהה **מעדכנת את הליד** במקום ליצור כפילות. הערך משוכפל גם ל-`unique_id`
כדי ש-`upsertLead` הקיים ימשיך לזהות את השורות האלה.

**מסך הייבוא** — הגדרות → **ייבוא לידים** (`src/components/settings/ImportLeadsTab.jsx`):
בחירת קובץ CSV/Excel → זיהוי אוטומטי של העמודות לפי שמות הכותרות (עברית ואנגלית) → תיקון
ידני של המיפוי → תצוגה מקדימה → ייבוא עם פס התקדמות → דוח (נוצרו / עודכנו / אנשי קשר חדשים /
שויכו לקיימים / נכשלו) + הורדת CSV של השורות שנכשלו. יש גם טבלת היסטוריית ייבואים.

**למה זה יעמוד ב-50k** — הדפדפן רק מפרסר ומעלה (הכנסות של 500 שורות לבקשה ל-`lead_import_rows`).
כל ההתאמה והכתיבה קורות בתוך `process_lead_import()` בצד השרת, באצוות של 500, כשהמצב שמור
בטבלה ולא בטאב. זאת לעומת המנגנון הקיים שיוצר שורה-אחת-בכל-פעם מתוך קומפוננטת React
(~4.4 שורות/שנייה). רשימת העמודות ב-INSERT מסוננת מול `information_schema` בזמן ריצה, כך
שעמודה אופציונלית שלא קיימת בפרודקשן מדולגת במקום להפיל את הייבוא — אותו עיקרון כמו
`writeWithSchemaResilience` ב-`src/api/entities.js`.

**מה שהמיגרציה לא עושה:** לא מוחקת כלום, ולא מוסיפה UNIQUE על `customers.phone_normalized`
אם כבר יש כפילויות — במקרה כזה היא רושמת NOTICE עם השאילתה לאיתורן וממשיכה. המיזוג
דורש את מפת ה-FK משלב 0.

**backfill ללידים הקיימים:** `SELECT public.backfill_lead_contacts(2000);` — להריץ שוב ושוב
עד שמחזיר 0. לא רץ בתוך המיגרציה כי 104k קריאות היו חורגות מה-timeout.
