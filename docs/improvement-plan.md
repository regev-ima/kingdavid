...
### E4. בדיקות עשן אוטומטיות — אין היום אף בדיקה במערכת
להקים חבילת Playwright מינימלית (5–7 תרחישים) שרצה ב-CI על כל PR: התחברות, יצירת ליד, יצירת הצעת מחיר עם מוצר+מיטה+תצורה, עריכת הצעה בפופ-אפ, יצירת הזמנה, סימון משימה כהושלמה. זו ההגנה הזולה ביותר מפני רגרסיות מהסוג שכבר קרה (שמירה שנשברת, ספירות שמתקלקלות). Chromium כבר זמין ב-CI של GitHub.

### E5. תפעול, גיבוי ושחזור (לפני מסירה ללקוח)
1. **גיבויים:** לוודא ב-Supabase שגיבוי יומי/PITR פעיל בהתאם לתוכנית, ולתעד נוהל שחזור קצר (מי, איך, כמה זמן אחורה). מיגרציות אינן הפיכות — הגיבוי הוא הרשת.
2. **ניטור כשלים:** GitHub Actions שנכשל (מיגרציה/פריסת פונקציות) — לוודא שמישהו מקבל התראה (הגדרות notifications), אחרת מיגרציה כושלת עוברת בשקט.
3. **Rollback לפרונט:** לתעד — Vercel → Deployments → Promote לפריסה קודמת (דקה אחת).
4. **הקשחת Auth בסופבייס (הגדרות, לא קוד):** אורך סיסמה מינימלי, הגנת סיסמאות דלופות, ולשקול MFA לחשבונות אדמין.
5. **rate-limit לנקודת הקצה הציבורית** של טופס השירות (service_request_* RPCs) — חשיפה נמוכה (טוקן UUID) אבל אין שום מגבלה כיום.

### E6. היגיינת סביבות — ה-preview עובד מול DB הפרודקשן!
כל בדיקה ב-preview יוצרת לידים/הצעות/הזמנות **אמיתיים** בפרודקשן. לפני מסירה:
1. לנקות את נתוני הבדיקה שהצטברו (לידים/הצעות/הזמנות בדיקה — לזהות לפי שמות בדיקה/טלפונים פיקטיביים, למחוק בזהירות עם גיבוי לפני).
2. להחליט קדימה: פרויקט Supabase נפרד ל-staging (נקי אבל דורש תחזוקת סנכרון סכימה), או להמשיך במודל הנוכחי עם נוהל ניקוי קבוע. החלטת בעלים.

### E7. תיעוד למסירה
1. **מדריך נציג** קצר (עמוד-שניים): זרימת ליד→משימות→הצעה→הזמנה, איתור ליד, שיבוץ משמרות.
2. **Runbook אדמין:** הוספת/הזמנת משתמש והרשאות (נהל נציג), הגדרות 019 SMS, ימי סגירה, קטלוג מוצרים + תצורת מיטות, ייבוא מ-Sheets.
3. סקירת פרטיות בסיסית: אילו נתוני לקוח נשמרים ומי רואה אותם (רלוונטי לחוק הגנת הפרטיות).
...
| שלב | סעיפים | הערות |
|---|---|---|
| 0 | **מיזוג PR #276** + אימות כל ה-workflows (C6) | לפני הכל — עשרות תיקונים ומיגרציות ממתינים שם; בלעדיו אי-אפשר לאמת חצי מהתוכנית |
...
| 7 | B1 (נעילת כתיבה בלבד), B6, E2, E4 | הקשחות + מומלץ-למסירה + בדיקות עשן |
| 8 | A10–A11, C3, C4, E3 | ניקיונות: SalesDashboard, קוד מת, קבצי ענק |
| 9 | E5, E6, E7 | תפעול/גיבוי, ניקוי נתוני בדיקה, תיעוד — צמוד למסירה |
